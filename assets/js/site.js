...
/* =============================================================
   Social marks
   Brand glyphs for the profile links, inline so they inherit
   colour and need no network request. 24x24 viewBox throughout.
   ============================================================= */

var SOCIAL_MARKS = {
  x: {
    tint: "#e7e9ee",
    body: '<path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68' +
          'l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>'
  },
  linkedin: {
    tint: "#4aa3e8",
    body: '<path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445' +
          '-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 ' +
          '2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 ' +
  ...
  telegram: {
    tint: "#3fb2e3",
    body: '<path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0' +
          ...
          '-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247' +
          '-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14' +
          '-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374' +
          '-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 ' +
          '4.025-1.627 4.476-1.635z"/>'
  },
  github: {
    tint: "#c9d1d9",
    body: '<path d="M12 .3a12 12 0 0 0-3.8 23.4c.6.1.8-.3.8-.6v-2c-3.3.7-4-1.6-4-1.6-.6-1.4-1.4-1.8' +
          '-1.4-1.8-1.1-.8.1-.8.1-.8 1.2.1 1.9 1.2 1.9 1.2 1.1 1.9 2.9 1.3 3.6 1 .1-.8.4-1.3.8-1.6' +
          '-2.7-.3-5.5-1.3-5.5-5.9 0-1.3.5-2.4 1.2-3.2-.1-.3-.5-1.5.1-3.2 0 0 1-.3 3.3 1.2a11.5 ' +
          '11.5 0 0 1 6 0C17.3 4.7 18.3 5 18.3 5c.6 1.7.2 2.9.1 3.2.8.8 1.2 1.9 1.2 3.2 0 4.6-2.8 ' +
          '5.6-5.5 5.9.4.4.8 1.1.8 2.2v3.3c0 .3.2.7.8.6A12 12 0 0 0 12 .3z"/>'
  },
  email: {
    tint: "#8fb8a6",
    body: '<path d="M2 5.5A2.5 2.5 0 0 1 4.5 3h15A2.5 2.5 0 0 1 22 5.5v13a2.5 2.5 0 0 1-2.5 2.5h-15' +
          'A2.5 2.5 0 0 1 2 18.5zM4.7 5l7.3 5.6L19.3 5zM20 6.9l-7.4 5.7a1 1 0 0 1-1.2 0L4 6.9v11.6' +
          'c0 .3.2.5.5.5h15c.3 0 .5-.2.5-.5z"/>'
  }
};

/* icon key from an explicit field, else guessed from the label */
function socialMark(l) {
  var key = (l.icon || l.label || "").toLowerCase().replace(/[^a-z]/g, "");
  var m = SOCIAL_MARKS[key];
  if (!m) return "";
  return '<svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">' + m.body + "</svg>";
}

function socialTint(l) {
  var key = (l.icon || l.label || "").toLowerCase().replace(/[^a-z]/g, "");
  return (l.tint || (SOCIAL_MARKS[key] && SOCIAL_MARKS[key].tint) || "");
}
  ...
  if (row) {
    row.innerHTML =
      ...
      (p.links || []).map(function (l) {
        var tint = socialTint(l);
        var style = tint ? ' style="--tint:' + esc(tint) + '"' : "";
        var body  = socialMark(l) + "<span>" + esc(l.label) + "</span>";

        if (!l.url) return '<span class="pb-link"' + style + ">" + body + "</span>";
        /* off-site links open in a new tab so the profile stays put */
        var ext = /^https?:/i.test(l.url) ? ' target="_blank"' : "";
        return '<a class="pb-link" href="' + esc(l.url) + '"' + ext + style +
               ' rel="noopener noreferrer">' + body + "</a>";
      }).join("");
  }
